...
for(i = 0; i < 10; i++){
    let html = `<div class="element" style="left: ${i * 10}%; width: 10%; height: ${(i + 1) * 10}%;"></div>`;
    arrEl.push(html);
}
shuffleArray();
const updateElements = function() {
    for(i = 0; i < 10; i++)containerSort.insertAdjacentHTML("afterbegin", arrEl[i]);
};
updateElements();
...
console.log(allEl);
console.log(Number.parseInt(allEl[0].style.height, 10));
console.log(Number.parseInt(allEl[1].style.height, 10)); // setInterval(function () {
 //   const now = new Date();
 //   console.log(now);
 // }, 1000);
